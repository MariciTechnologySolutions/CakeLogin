...
define('DBXPATH','/Us-Acquisitions');
...
class Dropbox {
	...
	private $rootPath;
	private $useCurl;
        private $dboxConfig = array(
                    'app_key' => "jrzvwcqm9wwo4nd", 
                    'app_secret' => "cokjxdb98aqq33a",
                    'dropboxORsandbox' => "dropbox",
                );
	
	function __construct (){
            if(empty($this->dboxConfig['app_key']))
                    throw new DropboxException("App Key is empty!");
            $this->consumerToken = array('t' => $this->dboxConfig['app_key'], 's' => $this->dboxConfig['app_secret']);		
            $this->locale = "en";
            $this->rootPath = $this->dboxConfig['dropboxORsandbox'];
...
// Dropbox API Functions
	...
	*/
	...
	public function GetLink($dropbox_file, $preview=true, $short=true, &$expires=null)
	{
		if(is_object($dropbox_file) && !empty($dropbox_file->path)) $dropbox_file = $dropbox_file->path;
	...
	public function GetRevisions($dropbox_file, $rev_limit=10)
	{
		if(is_object($dropbox_file) && !empty($dropbox_file->path)) $dropbox_file = $dropbox_file->path;
	...
	public function Search($path, $query, $file_limit=1000, $include_deleted=false)
	{
...
// Manage Tokens
        ...
        private function manageTokens()
        {
                    ...
                    if(!$this->IsAuthorized()){
                            ...
                            $auth_url = $this->BuildAuthorizeUrl($return_url);
	 ...
	 */
        private function GetRequestToken($get_new_token=false)
	{
	 ...
	 * @return string URL
	 */
	private function BuildAuthorizeUrl($return_url)
	{
	 ...
	 */
	private function GetAccessToken($request_token = null)
	{
	...
	private function IsAuthorized()
	{
        ...
        {
                if(!file_put_contents(dirname(__FILE__)."/tokens/$name.token", serialize($token)))
                        die('<br />Could not store token! <b>Make sure that the directory `tokens` exists and is writable!</b>');
        }

        private function load_token($name)
        {
                if(!file_exists(dirname(__FILE__)."/tokens/$name.token")) return null;
                return @unserialize(@file_get_contents(dirname(__FILE__)."/tokens/$name.token"));
        }

        private function delete_token($name)
        {
                @unlink(dirname(__FILE__)."/tokens/$name.token");
        }
        
//End Manage Tokens	
// ##################################################
        
        private function SetUseCUrl($use_it)
	{
